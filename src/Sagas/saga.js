...
function* fetchSongsWorker(action) {
  try {
    ...
    const response = yield call(searchSongsApi, action.searchedTerm);
    const filteredSongs = filterResultBySongs(response);
    yield put({
      type: Types.UPDATE_SONGS,
      songs: filterResultBySongs(filteredSongs),
    });
    if (filteredSongs && filteredSongs.length) { // For Empty Array Do Not Remove Searched Term From Searched Field 
